...
typedef OnLoadChapterEvent = Future<List<String>> Function(
    int chapter, bool isCurChapter);

/// 加载章节段落
typedef OnBuildSpans = Future<List<List<InlineSpan>>>? Function(
    int chapter, List<String> paragraphs);

typedef ChapterWidgetBuilder = Widget Function(BuildContext context,
    int chapter, int page, int maxPage, List<InlineSpan> spans);
...
class ChapterPageController extends PageController {
  ...
  int get curPage =>
      initialPage == 0 ? page!.round() : page!.round() - initialPage;
  ...
  _cacheChapter(int chapter) async {
    ...
    if (_spans == null || _spans.isEmpty) return;
  ...
  Future<bool> toChapter(int chapter, {bool toFirst = false}) async {
    ...
    _loadingChapter = -1;
    if (toFirst == true && curPage != 0) this.jumpToPage(initialPage);
    return true;
  }
  ...
  initChapter() async {
    ...
    // 缓存
    if (_prev.isEmpty) _cacheChapter(chapter - 1);
    if (_next.isEmpty) _cacheChapter(chapter + 1);
    if (_initialPage > 0) this.jumpToPage(initialPage + _initialPage);
  }
  ...
  toNextChapter() async {
    ...
    if (!await toChapter(chapter + 1)) {
      _prev = _tmpPrev;
      _next = _tmpNext;
      animateToPage(initialPage + pageCount - 1,
...
class _ChapterPageViewState extends State<ChapterPageView> {
  ...
  Widget build(BuildContext context) {
    ...
    final ScrollPhysics physics = widget.pageSnapping
        ? _kPagePhysics.applyTo(BouncingScrollPhysics())
      ...
      onNotification: (ScrollNotification notification) {
        if (notification.depth == 0 &&
          ...
          if (currentPage != _lastReportedPage) {
            ...
            final _realPage = widget.controller.initialPage == 0
                ? currentPage
              ...
              slivers: <Widget>[
                SliverFillViewport(
                  ...
                  delegate: SliverChildBuilderDelegate((context, index) {
                    final _page = widget.controller.initialPage == 0
                        ? index
                    ...
                    if (_page < 0) {
                      return widget.builder(
                          context,
                          widget.controller.chapter - 1,
                          widget.controller._prev.length - 1,
                          widget.controller._prev.length,
                          widget.controller._prev.isEmpty
                              ? []
                              : widget.controller._prev.last);
                    }
                    if (_page >= widget.controller.pageCount) {
                      return widget.builder(
                          context,
                          ...
                          widget.controller._next.isEmpty
                              ? []
                              : widget.controller._next.first);
                    }
                    return widget.builder(
                        context,
                        widget.controller.chapter,
                        ...
                        _page >= widget.controller.pageCount
                            ? []
                            : widget.controller.spans![_page]);
                  }, childCount: null),
                ),
              ],
            );
